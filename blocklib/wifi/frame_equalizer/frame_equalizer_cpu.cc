...
#include "frame_equalizer_cpu.h"
#include "frame_equalizer_cpu_gen.h"

namespace gr {
namespace wifi {

frame_equalizer_cpu::frame_equalizer_cpu(block_args args)
    : INHERITED_CONSTRUCTORS, d_freq(args.freq), d_bw(args.bw)
{
...
work_return_code_t
frame_equalizer_cpu::work(std::vector<block_work_input_sptr>& work_input,
                          std::vector<block_work_output_sptr>& work_output)
{
    ...
    dout << "FRAME EQUALIZER: input " << ninput_items << "  output " << noutput_items
         << std::endl;
    ...
    while ((i < ninput_items) && (o < noutput_items)) {
        ...
        if (tags.size()) {
            ...
            auto tag_val = pmtf::get_as<double>(tags.front()["wifi_start"]);
...
bool frame_equalizer_cpu::decode_signal_field(uint8_t* rx_bits)
{
...
void frame_equalizer_cpu::deinterleave(uint8_t* rx_bits)
{
    for (int i = 0; i < 48; i++) {
...
bool frame_equalizer_cpu::parse_signal(uint8_t* decoded_bits)
{
...
const int frame_equalizer_cpu::interleaver_pattern[48] = {
