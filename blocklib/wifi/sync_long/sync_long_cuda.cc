...
#include "sync_long_cuda.h"
#include "sync_long_cuda_gen.h"
...
sync_long_cuda::sync_long_cuda(block_args args) : INHERITED_CONSTRUCTORS

{
    sync_long_cu::get_block_and_grid(&d_min_grid_size, &d_block_size);
                                        ...
                                        std::vector<block_work_output_sptr>& work_output)
{
    ...
    while (true) {
        if (d_state == FINISH_LAST_FRAME) {
            ...
            sync_long_cu::exec_remove_cp_freqcorr((cuFloatComplex*)in + nconsumed,
