...
using namespace gr::wifi;

constellation_bpsk::sptr
constellation_bpsk::make() {
	return std::make_shared<constellation_bpsk>();
}
...
constellation_16qam::make() {
	return std::make_shared<constellation_16qam>();
}
...
constellation_64qam::make() {
	return std::make_shared<constellation_64qam>();
}
...
unsigned int
constellation_64qam::decision_maker(const gr_complex *sample) {
	unsigned int ret = 0;
	const float level = sqrt(float(1/42.0));
	float re = sample->real();
	float im = sample->imag();
